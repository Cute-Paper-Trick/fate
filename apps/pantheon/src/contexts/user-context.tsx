...
const getFromLocalStorage = (
  key: string,
  fallback: () => string,
  isServer: boolean = globalThis.window === undefined,
): string => {
  if (isServer) {
    return fallback();
  }
...
export function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    globalThis.localStorage.setItem('_tiptap_username', user.name);
    globalThis.localStorage.setItem('_tiptap_color', user.color);
    globalThis.localStorage.setItem('_tiptap_user_id', user.id);
  }, [user]);
