...
const FileCornerIcon: React.FC = () => (
  <svg
    width="10"
    height="10"
    className="tiptap-audio-upload-dropzone-rect-secondary"
    viewBox="0 0 10 10"
    fill="currentColor"
    xmlns="http://www.w3.org/2000/svg"
  >
...
const AudioUploadDragArea: React.FC<AudioUploadDragAreaProps> = ({
  ...
  return (
    <div
      className={`tiptap-audio-upload-drag-area ${isDragActive ? "drag-active" : ""} ${isDragOver ? "drag-over" : ""}`}
...
const DropZoneContent: React.FC<{ maxSize: number; limit: number }> = ({
  ...
  <>
    ...
    <div className="tiptap-audio-upload-content">
      ...
      <span className="tiptap-audio-upload-subtext">
        Maximum {limit} audio file{limit === 1 ? "" : "s"}, {maxSize / 1024 / 1024}MB
        each.
      </span>
    </div>
  </>
        ...
        <div className="tiptap-audio-upload-previews">
          {fileItems.length > 1 && (
            <div className="tiptap-audio-upload-header">
              <span>Uploading {fileItems.length} audio files</span>
              <Button
                type="button"
                data-style="ghost"
                onClick={(e) => {
