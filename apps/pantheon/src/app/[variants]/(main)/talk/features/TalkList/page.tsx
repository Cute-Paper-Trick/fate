...
export default function TopicList() {
  ...
  const { paginationQuery: paginationQuery, fetchNextPage: fetchNextPageStatus } =
    usePaginationStatus({
      type: type,
      currentUserId: currentUserId,
      enabled: !isHome && !!currentUserId,
    });
  const paginationQueryLength = paginationQuery.data?.pages[0]?.total;
  const hasMore = isHome ? currentListQuery.hasNextPage : topicList.length < paginationQueryLength!;
