...
export interface LiveSpectrogramRef {
  stopMinimalCleanup: () => void;
}

interface LiveSpectrogramProps {
  classId: string;
  onRecordingStopped: (classId: string) => void;
}

const LiveSpectrogram = React.forwardRef<LiveSpectrogramRef, LiveSpectrogramProps>(
  ({ classId, onRecordingStopped }, ref) => {
          ...
          if (avg < 3) {
            ...
            if (lowSignalCounterRef.current >= 120) {
              ...
              if (now - lastWarningTimeRef.current > 1000) {
                message.warning(
                  t('classifier.audio.warring.low', '音量过低，可能导致样本提取失败'),
                );
                lastWarningTimeRef.current = now;
              }
    ...
    const stopMinimalCleanup = React.useCallback(() => {
      setRecording(false);
      setRecEnding(true);
      setDuration(0);
    ...
    React.useImperativeHandle(ref, () => ({
      stopMinimalCleanup: stopMinimalCleanup,
    }));
